...
function normalizeCsvRow(raw: Record<string, unknown>): Record<string, unknown> {
  ...
  return {
    client_id: firstNonEmpty(raw, ['client_id', 'Client ID']),
    login_url: firstNonEmpty(raw, ['login_url', 'Login URL']),
    login_id: firstNonEmpty(raw, ['login_id', 'Login ID', 'email']),
    password: firstNonEmpty(raw, ['password', 'Password']),
    client_source: firstNonEmpty(raw, ['client_source', 'Client Source', 'source']),
    client_name: firstNonEmpty(raw, ['client_name', 'Client Name']),
    date_from: firstNonEmpty(raw, ['date_from', 'Date From', 'Start Date', 'start_date']),
    date_to: dateTo,
    medium_url: firstNonEmpty(raw, ['medium_url', 'Medium URL']),
    lead_url: firstNonEmpty(raw, ['lead_url', 'Lead URL']),
    year: firstNonEmpty(raw, ['year', 'Year']),
  };
}
